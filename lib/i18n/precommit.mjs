...
const excludeString = config.i18n.path_to_ignore ? `":(exclude)${config.i18n.path_to_ignore}"` : ''
const { stdout: stagedFilesNumber } = await execute(
  `git --no-pager diff --staged --name-only "${config.i18n.extract_from_pattern}" ${excludeString} | wc -l`
)
